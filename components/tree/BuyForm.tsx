...
type FormData = Partial<TreeBuyRequest>;
...
export function BuyForm() {
  ...
  const { hasSavedData } = useFormPersistence("buy", formData, currentStep);

  // Cargar datos guardados al montar
  useEffect(() => {
    const saved = loadFormData("buy");
    if (saved) {
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      ...
      case 4: return !!formData.rentaMensual;
      case 5: return !!formData.complementarRenta;
      case 6: return !!formData.situacionFinanciera;
      case 7: return !!formData.capacidadAhorro;
      case 8: return !!formData.preferenciaContacto;
  ...
  const handleNext = () => {
    if (!validateStep(currentStep)) {
      setError("Por favor completa este campo antes de continuar");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Validar con Zod
      const validatedData = TreeBuyRequestSchema.parse(normalizedData);
  ...
  const getEstimatedTime = (): number => {
    const stepTimes: Record<number, number> = {
      ...
      3: 5,   // Email
      4: 5,   // Renta mensual
      5: 5,   // Complementar renta
      6: 5,   // Situación financiera
      7: 5,   // Capacidad ahorro
      8: 5,   // Preferencia contacto
      9: 3,   // Consentimiento
    };
  ...
  if (isSuccess) {
    const whatsappUrl = buildWhatsAppUrl({
      message: `Hola, completé el formulario de compra. Mi nombre es ${formData.name}.`,
    });
          ...
          <Card className="rounded-2xl border-border bg-card">
            <CardContent className="pt-6 sm:pt-8">
              ...
              >
                ...
                <div className="flex flex-col gap-3 w-full mt-4 sm:mt-6">
                  <Button
                    onClick={() => {
                      track(ANALYTICS_EVENTS.CTA_SCHEDULE_CLICK, { flow: "buy" });
                      window.location.href = "/agendamiento";
                    }}
                    className="w-full min-h-[44px] rounded-xl bg-brand-violet hover:bg-brand-violet/90 text-white"
                    size="lg"
                  >
                  ...
                  {whatsappUrl && (
                    <Button
                      ...
                      variant="secondary"
                      className="w-full min-h-[44px] rounded-xl bg-brand-aqua hover:bg-brand-aqua/90 text-black"
                      size="lg"
                    >
                      WhatsApp directo
                    </Button>
        ...
        <Card className="rounded-2xl border-border bg-card shadow-sm">
          <CardHeader className="pb-4 sm:pb-6">
            <CardTitle className="text-lg sm:text-xl text-text">
              {currentStep === 1 && "Invierte en tu futuro"}
              {currentStep === 2 && "Datos de contacto"}
              {currentStep === 3 && "Correo electrónico"}
              {currentStep === 4 && "¿Cuál es tu renta mensual promedio?"}
              {currentStep === 5 && "¿Puedes complementar renta?"}
              {currentStep === 6 && "¿Cuál es tu situación financiera?"}
              {currentStep === 7 && "¿Cuál es tu capacidad de ahorro mensual?"}
              {currentStep === 8 && "¿Cómo prefieres que te contactemos?"}
              {currentStep === 9 && "Consentimiento"}
            </CardTitle>
            {currentStep === 1 && (
              <p className="text-sm sm:text-base text-subtext mt-2">
                Te ayudamos a encontrar la propiedad ideal para tu inversión
              </p>
            )}
          </CardHeader>
          <CardContent>
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {currentStep === 4 && (
                  <div>
                    <Label className="text-sm sm:text-base text-text font-medium mb-3 block">Renta mensual promedio</Label>
                    <div className="mt-2 space-y-2 sm:space-y-3">
                      {RENTA_MENSUAL_OPTIONS.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => updateField("rentaMensual", option.value)}
                          className={`w-full text-left p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 min-h-[44px] text-sm sm:text-base font-medium ${formData.rentaMensual === option.value
                ...
                {currentStep === 5 && (
                  <div>
                    <Label className="text-sm sm:text-base text-text font-medium mb-3 block">¿Puedes complementar renta?</Label>
                    <div className="mt-2 space-y-2 sm:space-y-3">
                      {COMPLEMENTAR_RENTA_OPTIONS.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => updateField("complementarRenta", option.value)}
                          className={`w-full text-left p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 min-h-[44px] text-sm sm:text-base font-medium ${formData.complementarRenta === option.value
                            ? "border-brand-violet bg-brand-violet/10 dark:bg-brand-violet/20 text-brand-violet dark:text-brand-aqua shadow-sm"
                            : "border-border hover:border-brand-violet/50 dark:hover:border-brand-violet/40 bg-card hover:bg-surface dark:hover:bg-surface text-text"
                            } focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-violet focus-visible:ring-offset-2`}
                        >
                ...
                {currentStep === 6 && (
                  <div>
                    <Label className="text-sm sm:text-base text-text font-medium mb-3 block">Situación financiera</Label>
                    <div className="mt-2 space-y-2 sm:space-y-3">
                      {SITUACION_FINANCIERA_OPTIONS.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => updateField("situacionFinanciera", option.value)}
                          className={`w-full text-left p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 min-h-[44px] text-sm sm:text-base font-medium ${formData.situacionFinanciera === option.value
                            ? "border-brand-violet bg-brand-violet/10 dark:bg-brand-violet/20 text-brand-violet dark:text-brand-aqua shadow-sm"
                            : "border-border hover:border-brand-violet/50 dark:hover:border-brand-violet/40 bg-card hover:bg-surface dark:hover:bg-surface text-text"
                            } focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-violet focus-visible:ring-offset-2`}
                        >
                ...
                {currentStep === 7 && (
                  <div>
                    <Label className="text-sm sm:text-base text-text font-medium mb-3 block">Capacidad de ahorro mensual</Label>
                    <div className="mt-2 space-y-2 sm:space-y-3">
                      {CAPACIDAD_AHORRO_OPTIONS.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => updateField("capacidadAhorro", option.value)}
                          className={`w-full text-left p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 min-h-[44px] text-sm sm:text-base font-medium ${formData.capacidadAhorro === option.value
                            ? "border-brand-violet bg-brand-violet/10 dark:bg-brand-violet/20 text-brand-violet dark:text-brand-aqua shadow-sm"
                            : "border-border hover:border-brand-violet/50 dark:hover:border-brand-violet/40 bg-card hover:bg-surface dark:hover:bg-surface text-text"
                            } focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-violet focus-visible:ring-offset-2`}
                        >
                ...
                {currentStep === 8 && (
                  <div>
                    <Label className="text-sm sm:text-base text-text font-medium mb-3 block">Preferencia de contacto</Label>
                    <div className="mt-2 space-y-2 sm:space-y-3">
                      {PREFERENCIA_CONTACTO_OPTIONS.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => updateField("preferenciaContacto", option.value)}
                          className={`w-full text-left p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 min-h-[44px] text-sm sm:text-base font-medium ${formData.preferenciaContacto === option.value
                            ? "border-brand-violet bg-brand-violet/10 dark:bg-brand-violet/20 text-brand-violet dark:text-brand-aqua shadow-sm"
                            : "border-border hover:border-brand-violet/50 dark:hover:border-brand-violet/40 bg-card hover:bg-surface dark:hover:bg-surface text-text"
                            } focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-violet focus-visible:ring-offset-2`}
                        >
