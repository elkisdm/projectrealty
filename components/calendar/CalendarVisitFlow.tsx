...
const MOCK_SLOTS: AvailabilitySlot[] = [
    ...
    { start: asIsoDateTime('2024-01-15T12:00:00Z'), end: asIsoDateTime('2024-01-15T13:00:00Z'), available: false },
    { start: asIsoDateTime('2024-01-15T14:00:00Z'), end: asIsoDateTime('2024-01-15T15:00:00Z'), available: true },
    { start: asIsoDateTime('2024-01-15T15:00:00Z'), end: asIsoDateTime('2024-01-15T16:00:00Z'), available: true },
    { start: asIsoDateTime('2024-01-15T16:00:00Z'), end: asIsoDateTime('2024-01-15T17:00:00Z'), available: false },
    { start: asIsoDateTime('2024-01-15T17:00:00Z'), end: asIsoDateTime('2024-01-15T18:00:00Z'), available: true },
];
...
export default function CalendarVisitFlow({ building, unit, date, slots: initialSlots, googleCalendarId, icsUrl, className }: CalendarVisitFlowProps) {
    ...
    useEffect(() => {
        logger.log('🔄 useEffect ejecutándose...', { initialSlots, googleCalendarId, icsUrl });

        if (initialSlots) {
            logger.log('✅ Usando slots iniciales:', initialSlots);
        ...
        async function load() {
            try {
                ...
                if (!googleCalendarId && !icsUrl) {
                    logger.log('🔍 Cargando datos mock...');
                    // Simular delay de red
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    if (!cancelled) {
                        logger.log('✅ Datos mock cargados:', MOCK_SLOTS);
                        setSlots(MOCK_SLOTS);
                        setLoading(false);
                    }
                    return;
                ...
                logger.log('🌐 Haciendo fetch a API...');
                ...
                if (!cancelled) {
                    logger.log('✅ Datos de API cargados:', data.slots);
                    setSlots(data.slots ?? []);
                }
            } catch (error) {
