  ...
  const { data, isLoading, isFetching, error } = useQuery({
    ...
    queryFn: async (): Promise<SearchResultsResponse> => {
      try {
        ...
        // Log con información estructurada (solo si hay parámetros reales o es un error crítico)
        const hasParams = Object.values(params).some(val => val !== undefined && val !== null && val !== '');
        const isCriticalError = errorMessage.includes('400') || errorMessage.includes('500') || errorMessage.includes('429');
