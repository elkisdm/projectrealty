...
import { POST } from '@/app/api/contracts/validate/route';
...
jest.mock('@/lib/contracts/service', () => ({
  validateContractForTemplate: jest.fn().mockResolvedValue({
    valid: true,
    errors: [],
    warnings: [],
    missingPlaceholders: [],
...
describe('POST /api/contracts/validate', () => {
  test('returns validation payload', async () => {
    const req = new Request('http://localhost/api/contracts/validate', {
      method: 'POST',
      headers: {
