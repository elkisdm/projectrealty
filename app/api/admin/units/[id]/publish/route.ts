...
import { AdminUnitPublishSchema, adminError, adminOk } from "@lib/admin/contracts";
import { requireAdminSession } from "@lib/admin/guards";
import { publishAdminUnit } from "@lib/admin/repositories/units.repository";
...
export async function POST(
  ...
  try {
    ...
    const parsedBody = AdminUnitPublishSchema.safeParse(body);
    if (!parsedBody.success) {
      return adminError("validation_error", "Payload inválido para publicación", {
        status: 400,
        details: parsedBody.error.errors,
      });
    }

    const published = await publishAdminUnit({
      id: parsedParams.data.id,
      status: parsedBody.data.status || "published",
    });
    ...
    await logAdminActivity({
      ...
      metadata: {
        status: published.publicationStatus,
      },
    });

    return adminOk(published);
  } catch (error) {
